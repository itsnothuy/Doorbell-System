...
class DetectionFuser:
    ...
    def simple_voting_fusion(self, detector_results: Dict[str, List[FaceDetectionResult]], 
    ...
    def weighted_voting_fusion(self, detector_results: Dict[str, List[FaceDetectionResult]], 
                             detector_configs: Dict[str, DetectorConfig]) -> List[EnsembleDetection]:
    ...
    def confidence_weighted_fusion(self, detector_results: Dict[str, List[FaceDetectionResult]], 
                                 detector_configs: Dict[str, DetectorConfig]) -> List[EnsembleDetection]:
...
class EnsembleDetector(BaseDetector):
    ...
    def detect_faces(self, image: np.ndarray, 
                    performance_requirements: Optional[Dict[str, Any]] = None) -> Tuple[List[FaceDetectionResult], DetectionMetrics]:
        """
    ...
    def _detect_faces_enhanced(self, image: np.ndarray,
                              performance_requirements: Optional[Dict[str, Any]]) -> Tuple[List[FaceDetectionResult], DetectionMetrics]:
        ...
        try:
            ...
            metrics = DetectionMetrics(
                ...
                confidence=sum(r.confidence for r in final_results) / len(final_results) if final_results else 0.0
            )
    ...
    def _fuse_detections_new(self, detector_results: Dict[str, List[FaceDetectionResult]]) -> List[EnsembleDetection]:
        ...
        if self.fusion_strategy == FusionStrategy.SIMPLE_VOTING:
            ...
            return self.fusion_engine.weighted_voting_fusion(detector_results, self.detector_configs)
            ...
            return self.fusion_engine.confidence_weighted_fusion(detector_results, self.detector_configs)
            ...
            for detector_name, detections in detector_results.items():
                for detection in detections:
                    all_detections.append(self.fusion_engine._convert_detection_to_ensemble(detection, detector_name))
